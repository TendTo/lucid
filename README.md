...
[![lucid CI](https://github.com/TendTo/bazel-cpp-template/actions/workflows/lucid.yml/badge.svg)](https://github.com/TendTo/bazel-cpp-template/actions/workflows/lucid.yml)
...
bazel build --config=python --python_version=3.13 //pylucid:pylucid_wheel
```

```bash
# Run the python bindings tests
bazel test --config=python --python_version=3.13 //pylucid/tests/...
```

```bash
# Run the python bindings main application
bazel run //pylucid --config=python --python_version=3.13
```
