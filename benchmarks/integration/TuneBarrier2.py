...
def scenario_config(config: Configuration):
    ...
    val = np.logspace(-5, 2, num=4, endpoint=True, dtype=float)
    ...
    log.info(
        f"Initial: {estimator} with score {estimator.score(config.x_samples, feature_map(config.xp_samples))} and {estimator.score(new_x_samples, feature_map(new_xp_samples))}"
    )
    ############
    # LbfgsTuner
    ############
    tuner = LbfgsTuner(lb=(1e-5, 1e-5), ub=(1e5, 1e5), parameters=LbgsParameters(min_step=0, linesearch=5))
    estimator.fit(
        config.x_samples,
        config.xp_samples,
        tuner=tuner,
    ...
    estimator.fit(config.x_samples, feature_map(config.xp_samples))
    log.info(
        f"LbfgsTuner: {estimator} with score {estimator.score(config.x_samples, feature_map(config.xp_samples))} and {estimator.score(new_x_samples, feature_map(new_xp_samples))}"
    )
    #############
    # GridSearchTuner
    #############
    tuner = GridSearchTuner(
        parameters={
            Parameter.REGULARIZATION_CONSTANT: [1e-8],
