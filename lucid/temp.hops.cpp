...
int main() {
  std::cout << "Running HOPS sampling example..." << std::endl;
  constexpr long numberOfSamples = 10'000;
  constexpr long thinning = 5;
  constexpr long numberOfCheckPoints = 5;
  constexpr std::size_t dims = 3;

  constexpr double covScale = 1e-2;
  ...
  auto gaussianSampler = hops::MarkovChainFactory::createMarkovChain(
      chainType, Arounded, b, startPointRounded, roundingTrafo,
      // we skipped shifting the polytopes chebyshev center to the origin in this example
      Eigen::VectorXd(Eigen::VectorXd::Zero(roundingTrafo.cols())), model);
  ...
  auto uniform_results_writer =
      hops::FileWriterFactory::createFileWriter("uniform_sampling_results", hops::FileWriterType::CSV);
  auto gaussian_results_writer =
      hops::FileWriterFactory::createFileWriter("gaussian_sampling_results", hops::FileWriterType::CSV);
  ...
  for (long checkPoint = 0; checkPoint < numberOfCheckPoints; ++checkPoint) {
    ...
    uniform_results_writer->write(
        "acceptance_rates",
        std::vector<double>{std::reduce(acceptanceRates.begin(), acceptanceRates.end(), 0.) / acceptanceRates.size()});
