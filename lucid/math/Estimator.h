...
class Estimator : public Parametrizable {
 public:
  explicit Estimator(std::unique_ptr<tuning::Tuner>&& tuner = nullptr);
  /**
   ...
   * @return reference to the fitted estimator
   */
  virtual Estimator& fit(ConstMatrixRef training_inputs, ConstMatrixRef training_outputs,
                         const tuning::Tuner& tuner) = 0;
  /**
