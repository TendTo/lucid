 ...
 * @copyright https://gist.github.com/tesch1/1126425eb7cb1dfea35c9f0480111908
 * @licence BSD 3-Clause License
 * @file
 * Eigen extensions and expressions.
 * Taken from https://stackoverflow.com/questions/46077242/eigen-modifyable-custom-expression/46301503#46301503
 */
...
namespace internal {
template <typename XprType, typename RowIndices, typename ColIndices>
struct traits<CircShiftedView<XprType, RowIndices, ColIndices>> : traits<XprType> {
  enum {
...
class CircShiftedView
    : public CircShiftedViewImpl<XprType, RowShift, ColShift, typename internal::traits<XprType>::StorageKind> {
 public:
  typedef
      typename CircShiftedViewImpl<XprType, RowShift, ColShift, typename internal::traits<XprType>::StorageKind>::Base
          Base;
  ...
  CircShiftedView(XprType& xpr, const T0& rowShift, const T1& colShift)
    ...
    for (auto c = 0; c < xpr.cols(); ++c)
      assert(std::abs(helper::index_if_necessary(m_rowShift, c)) <
             m_xpr.rows());  // row shift must be within +- rows()-1
    for (auto r = 0; r < xpr.rows(); ++r)
      assert(std::abs(helper::index_if_necessary(m_colShift, r)) <
             m_xpr.cols());  // col shift must be within +- cols()-1
  }
  ...
  /** \returns the nested expression */
  typename internal::remove_reference<XprType>::type& nestedExpression() { return m_xpr.const_cast_derived(); }

  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE Index getRowIdx(Index row, Index col) const {
    Index R = m_xpr.rows();
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE Index getColIdx(Index row, Index col) const {
    Index C = m_xpr.cols();
...
class CircShiftedViewImpl : public internal::generic_xpr_base<CircShiftedView<XprType, RowIndices, ColIndices>>::type {
 public:
...
#pragma GCC diagnostic ignored "-Wdeprecated-enum-enum-conversion"
  enum {
    CoeffReadCost = (evaluator<ArgType>::CoeffReadCost + NumTraits<Index>::AddCost /* for comparison */
...
#pragma GCC diagnostic pop
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE CoeffReturnType coeff(Index idx) const {
    if (m_xpr.cols() == 1) return m_argImpl.coeff(m_xpr.getRowIdx(idx, 1), 1);
    if (m_xpr.rows() == 1) return m_argImpl.coeff(1, m_xpr.getColIdx(1, idx));
