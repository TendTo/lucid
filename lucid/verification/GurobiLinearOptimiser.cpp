...
bool GurobiLinearOptimiser::solve(ConstMatrixRef f0_lattice, ConstMatrixRef fu_lattice, ConstMatrixRef phi_mat,
  ...
  // TODO(tend): we should make variable and constraint naming contingent on a debug flag
  ...
  model.addConstr(eta + T_ * c, GRB_LESS_EQUAL, gamma_).set(GRB_StringAttr_ConstrName, "eta+c*T<=gamma");
  ...
  for (Index row = 0; row < phi_mat.rows(); ++row) {
    ...
    model.addConstr(expr, GRB_GREATER_EQUAL, 0).set(GRB_StringAttr_ConstrName, fmt::format("B(x)>=hatxi[{}]", row));
    expr -= maxXX * maxXX_coeff;  // TODO(tend): use expr.remove(maxXX);
    expr += -maxXX;
    model.addConstr(expr, GRB_LESS_EQUAL, 0).set(GRB_StringAttr_ConstrName, fmt::format("B(x)<=maxXX[{}]", row));
  }
  ...
  for (Index row = 0; row < f0_lattice.rows(); ++row) {
    ...
    model.addConstr(expr, GRB_LESS_EQUAL, 0).set(GRB_StringAttr_ConstrName, fmt::format("B(x_0)>=hateta[{}]", row));
    expr -= -fctr1 * eta - fctr2 * minX0;  // TODO(tend): use expr.remove(eta); expr.remove(minX0);
    expr += -minX0;
    model.addConstr(expr, GRB_GREATER_EQUAL, 0).set(GRB_StringAttr_ConstrName, fmt::format("B(x_0)>=minX0[{}]", row));
  }
  ...
  for (Index row = 0; row < fu_lattice.rows(); ++row) {
    ...
    expr -= -fctr2 * maxXU;  // TODO(tend): use expr.remove(maxXU);
    expr += -maxXU;
    model.addConstr(expr, GRB_LESS_EQUAL, 0).set(GRB_StringAttr_ConstrName, fmt::format("B(x_u)<=maxXU[{}]", row));
  }
  ...
  for (Index row = 0; row < mult.rows(); ++row) {
    ...
    model.addConstr(expr, GRB_LESS_EQUAL, kushner_rhs)
        .set(GRB_StringAttr_ConstrName, fmt::format("B_plus(x)-B(x)<=hatDelta[{}]", row));
    expr -= -fctr1 * (c)-fctr2 * minDelta;  // TODO(tend): use expr.remove(c); expr.remove(minDelta);
    expr += -minDelta;
    model.addConstr(expr, GRB_GREATER_EQUAL, 0)
        .set(GRB_StringAttr_ConstrName, fmt::format("B_plus(x)-B(x)>=minDelta[{}]", row));
  }
