...
class TestGaussianKernel : public ::testing::Test {
 protected:
  ...
  TestGaussianKernel() : sigma_f_{2.01}, sigma_l_{get_sigma_l()}, kernel_{sigma_f_, get_sigma_l()} {}
...
TEST_F(TestGaussianKernel, VectorConstructor) {
  const GaussianKernel kernel{1.0, Vector::Constant(3, 1.0)};
...
TEST_F(TestGaussianKernel, DimensionConstructor) {
  const GaussianKernel kernel{1.0, 3.0, 4};
  EXPECT_EQ(kernel.sigma_f(), 1.0);
