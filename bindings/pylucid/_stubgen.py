...
def generate_stub_files(_pylucid_pyi):
    ...
    out_dir = os.path.dirname(os.path.dirname(_pylucid_pyi))
...
def copy_lib(lib: "list[str]", _pylucid_pyi: str):
    """Copy the library files from the original position to the execution sandbox

    Args:
        lib: list of library files to copy
        _pylucid_pyi: output _pylucid.pyi file
    """
    sandbox_dir = os.path.dirname(_pylucid_pyi)
    for f in lib:
...
def main():
    ...
    _pylucid_pyi, _init_pyi, py_typed = args.outs

    copy_lib(args.lib, _pylucid_pyi)
    generate_stub_files(_pylucid_pyi)
