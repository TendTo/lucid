...
The transitions are then fed into \lucid, either programmatically, via a script that generates them on the fly, or through a file.
...
These are mainly used to indicate the bounds of the whole state space $\X$, the initial set $\X_0$ and the unsafe set $\X_U$ when defining the safety specification.
...
\lucid provides the option to employ the \dreal SMT solver~\cite{dreal} to verify the barrier formally if the system dynamics are known.
