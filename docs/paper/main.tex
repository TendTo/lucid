...
\title{Delta-Complete Linear Programming}
...
Lucid is in implemented in C++ and it is meant to be used as a library.
...
More technical details about the implementation can be found in the documentation at \url{https://tendto.github.io/lucid/}.
...
We briefly recall the definition of a CBC.
\begin{definition}[Control barrier certificate (CBC)]\label{def:cbc}
  A function $\B:\X\rightarrow\R_{\geq 0}$ is called a CBC of an MDP $\M=(\X,\X_0,\U,\Tr)$ with reference to an unsafe set $\X_u$, if we have
  \begin{itemize}
...
\end{definition}
If one can find a CBC for a system $\M$, then, a lower bound on the probability of $\M$ being safe is given by the following proposition due to \cite{kushner1967stochastic}.
...
When verifying the safety of a system, \lucid will search for a CBC $\B$ that satisfies the conditions in Definition~\ref{def:cbc}.
...
\end{align*}
For an upper bound $\forall x\in\X: f(x) \leq \mathrm{ub}$, we obtain analogous constraints of the form
\begin{equation*}
  \mathrm{max}_{\hat{N}}^\X \geq \frac{2}{C_{\hat{N}}+1}\mathrm{ub} + \frac{C_{\hat{N}}-1}{C_{\hat{N}}+1}\mathrm{min}_{\hat{N}}^\X.
\end{equation*}
