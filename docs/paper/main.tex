...
\begin{itemize}
    ...
    \item \texttt{LBFGSTuner}: borrowing the technique from Gaussian processes, we optimize the \hp to maximize the \emph{log marginal likelihood}, defined as
          \begin{equation}
              ...
              \end{split}
          \end{equation}
          where $\theta$ is a tuple of learnable hyperparameters of the \estimator determined by the user.
...
Since we know the expected behavior of the example system \EC{we should refer to the system (its equation?)}, we can run the formal validation to confirm that the barrier $\B$ is indeed a \gls{cbc} for the system $\S$.
...
To start the \gls{gui}, make sure to have installed the necessary dependencies with 
\begin{lstlisting}[language=bash,style=nonumber]
pip install pylucid[gui] ...
\end{lstlisting}
and run the command \lstinline|pylucid-gui|.
A browser tab containing the interface shown in Figure~\ref{fig:gui} will automatically open, while a local server will listen for requests coming from the \gls{gui}, computing and returning the results.
...
To define the configuration, we suggest using a \yaml or equivalent \json file.
\footnote{The Json schema for \yaml and \json files can be found at \url{https://lucidtoolsource.gitlab.io/lucid/configuration_schema.json}}\OS{I would personally avoid the word "scenario", as it could be misunderstood with an actual run of the system.}
