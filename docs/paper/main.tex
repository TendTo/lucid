...
\omnisafe is a comprehensive platform for the development of safe \gls{rl} algorithms~\citet{ji2024omnisafe}. However, safe \gls{rl} does generally only \emph{encourage} safer behavior, without providing any rigorous or quantified guarantees on the probability of the absence of unsafe behavior.
As a popular working principle for certifying safety, \glspl{cbc}~\cite{prajna2006barrier} are at the basis of tools such as \trust~\cite{gardner2025trust} (supports only polynomial dynamics) and \fossil~\cite{edwards2024fossil} (model-based), which are restricted to deterministic systems.
...
At its core, it learns \glspl{cbc} for unknown systems based solely on data, by constructing an uncertainty-aware estimator of the expected system behavior based on \glspl{cme}~\cite{muandet2017kernel}.
...
\begin{itemize}
    \item We introduce \lucid, a novel verification engine that learns control barrier certificates from data using kernel-based \glspl{cme}. \lucid uses a tractable reformulation of the verification problem via a finite Fourier expansion, enabling efficient barrier synthesis based on a linear program.
...
Even more, for unbounded stochasticity $w_t\sim p_w$, there is generally no deterministic answer (safe or not), but a safety probability $P_{\text{safe}}(\S^\pi)\in[0,1)$. Thus the goal is generally to estimate a \emph{lower bound} on the true safety probability $P_{\text{safe}}(\S^\pi)$.
...
For conditional probability distributions a similar concept exists: \glspl{cme} can be used to model the expected value of any \gls{rkhs} function $f\colon\X\rightarrow\R$ under a stochastic process such as $\S$~\cite{park2020measuretheoretic,muandet2017kernel}.
...
\begin{equation*}
    k_{XA}^N(x,a)\T\left[ K_{XU}^N+N \lambda I_N\right]^{-1}\! \Phi_{M,+}^N
    \approx
    \phi_M{\color{red}(x,a)}\T H,
\end{equation*}
where $\Phi_{M,+}^N:=[\phi_M(x_+^i)\T]_{i=1}^N$,
...
\begin{itemize}
    ...
    \item[(c)] $\forall x\in\X,\,\exists a\in\A\colon\,\phi_M{\color{red}(x,a)}\T (Hb -b) \leq  c.$
\end{itemize}
...
{\color{red}Instructions for installing \lucid are deferred to the appendix.}
...
\EC{we can remove all the explicit set intervals}
The results are shown in Figure~\ref{fig:CSBarr2}.
The kernel \hp have been selected through offline tuning to be $\sigma_f=1$ and $\sigma_l=[1.0, 5.39334]$.
...
\begin{figure}[ht]
    %\resizebox{\columnwidth}{!}{\import{figures}{barrier2.pdf}}%
    \includegraphics[trim={5cm 3cm 2cm 4cm},clip,width=\linewidth]{barrier2.pdf}
    \caption{\gls{cbc} synthesis for the \barrII benchmark.
        The blue circle represents the initial set $\X_0$, while the red one represents the unsafe set $\X_U$.
    }
    \label{fig:CSBarr2}
\end{figure}
...
Similarly, we also tested \lucid on the \barrIII benchmark adapted from \cite{abate2021fossil} with added stochastic noise.
The resulting nonlinear stochastic dynamics are
\begin{equation*}
    \begin{bmatrix}
        {x}_{1, t+1} \\
        {x}_{2, t+1}
