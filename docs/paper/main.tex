...
Furthermore, for unbounded stochasticity $w_t\sim p_w$, there is generally no yes/no answer to safety, but a safety probability $P_{\text{safe}}(\S^\pi)\in[0,1)$. Thus the goal is generally to estimate a \emph{lower bound} on the true safety probability $P_{\text{safe}}(\S^\pi)$.
...
For conditional probability distributions a similar concept exists: \glspl{cme} can be used to model the expected value of any \gls{rkhs} function $\new{g}\colon\X\rightarrow\R$ under a stochastic process such as $\S$~\cite{park2020measuretheoretic,muandet2017kernel}.
For the Gaussian kernel,
\begin{equation}
    k(x,x') := \sigma_f^2 \exp\left( -\tfrac{1}{2} (x-x')\T \Sigma\, (x-x') \right),\label{eq:gaussian_kernel}
\end{equation}
where $\Sigma:=\diag(\sigma_l)^{-2}$, with hyperparameters $\sigma_f,\sigma_l\in\R$, \new{the associated RKHS encompasses all smooth functions $g$}.
...
\end{align*}
Notably, $\B$ admits the linear form \new{$B(x)=\phi_M(x)\T b$}, with a truncated Fourier feature map $\phi_M\colon\X\rightarrow\R^{2M+1}$, parametrized by learnable spectral amplitudes
\begin{figure}
    \centering
    \includegraphics[width=\linewidth]{fourier_series.pdf}
    \caption{Spectral barrier certificate \new{$B(x)=\phi_M(x)\T b$}.}
...
\begin{equation*}
    ...
    \new{\varphi_M(x,a)}\T H,
\end{equation*}
where $\Phi_{M,+}^N:=[\phi_M(x_+^i)\T]_{i=1}^N$, \new{$\varphi_M(x,a)\colon\X\times\A\rightarrow\smash{\R^{2M+1}}$ a feature map augmenting $\phi_M(x)$}, and
...
\begin{itemize}
    ...
    \item[(c)] $\forall x\in\X,\,\exists a\in\A\colon\,\new{\varphi_M(x,a)}\T (Hb -b) \leq  c.$
\end{itemize}
\new{For the case where $a$ is provided by a given fixed policy $\pi$, $\varphi_M(x,a)$ reduces to $\phi_M(x)$.}
...
\Sadegh{space needs fixing before ".. to start ...".}
...
\OS{Thus the action $a$ in the previous section is replaced with the policy $\pi(x)$.}
...
For the running example, we obtain the barrier certificate shown in Figure~\ref{fig:example} for a time horizon of $T = 15$ certifying safety of the system in \eqref{eq:linear-system} with a probability of at least $93.73\%$ ($\eta = 0.013, \gamma=1.0, c = 0.003$). %\|B\| = 1.72

\begin{figure}
    \centering
    \resizebox{\columnwidth}{!}{\import{figures}{example.pgf}}
    \caption{Barrier certificate for the running example.
...
verify: true
\end{lstlisting}
Doing so confirms that $\B$ is indeed a valid \gls{cbc} for the system.
...
\begin{figure}
    ...
    The left half of the screen, \Sadegh{as indicated in this picture,} allows the user to provide the transition samples and the safety specification to verify.
...
To ascertain the performance of \lucid, we conduct a series of experiments, run on a Windows 10 machine with an AMD Ryzen 9 5950X 16-Core Processor @ 3.40 GHz, and 64 GB of RAM.
...
\Sadegh{we should combine Conclusion and Future extensions into one section and keep it in the main body. We should also mention sparse CME as a way of improving scalability...}

\new{This paper introduces \lucid, a novel }
