...
Their core functionality is build around the advances of deep learning, enabling systems to understand and reason in complex and human-like ways to produce a desired behavior.
...
Efforts to establishing trust in their safe operation have been largely focused on developing tools to verify the input--output behavior of neural networks (NNs) embedded in the systems~\cite{liu2021algorithms}.
...
There exist no tools capable of solving this problem with quantified guarantees.
%
For the setting where a model of the closed-loop system is available, \npinterval by \citet{harapanahalli2023forward} verifies safety of nonlinear systems with non-deterministic disturbance based on existing NN verification tools. See the references therein and the annual friendly competition by \citet{abate2024arch} for adjacent work.
%
Few data-driven approaches for stochastic systems exist. 
\omnisafe is a comprehensive platform for the development of safe reinforcement learning (RL) algorithms~\citet{ji2024omnisafe}. However, safe RL does generally only \emph{encourage} safer behavior, without providing any rigorous or quantified guarantees on the probability of the absence of unsafe behavior.
As a popular working principle for certifying safety, control barrier certificates (CBCs)~\cite{prajna2006barrier} are at the basis of tools such as \trust~\cite{gardner2025trust} (supports only polynomial dynamics) and \fossil~\cite{edwards2024fossil} (model-based), which are restricted to deterministic systems.
...
At its core, it learns CBCs for unknown systems based solely on data, by constructing an uncertainty-aware estimator of the expected system behavior based on \emph{conditional mean embeddings} (CMEs)~\cite{muandet2017kernel}.
...
For instance, \citet{schoen2024distributionally} learn CBCs for systems with polynomial dynamics and \citet{chen2025distributionally} assume systems with known deterministic component.
%
...
\begin{itemize}
    \item We introduce \lucid, a novel verification engine that learns control barrier certificates from data using kernel-based CMEs. \lucid uses a tractable reformulation of the verification problem via a finite Fourier expansion, enabling efficient barrier synthesis based on a linear program.
...
Section~\ref{sec:experiments} evaluates \lucid on a suite of benchmarks. Finally, Sections~\ref{sec:future_extensions} and \ref{sec:conclusion} provide future extensions and a conclusion.
...
\end{equation}
where every sample transition from $x^i\in\X$ to a successor $x^i_+\in\X$ is generated as a realization
\begin{equation*}
...
\lucid provides a solution to this problem by automating the steps in Figure~\ref{fig:steps}, outlined in the next sections.
...
Notably, the system dynamics only enter the barrier conditions through the conditional expectation in (c), and a \gls{cbc} thus only depends on the \emph{expected} system behavior, with no need to consider its concrete stochastic law.
...
For conditional probability distributions a similar concept exists: \glspl{cme} can be used to model the expected value of any RKHS function $f\colon\X\rightarrow\R$ under a stochastic process such as $\S$~\cite{park2020measuretheoretic,muandet2017kernel}.
...
It is common practice to robustify empirical estimates such as \eqref{eq:empiricalEstimate} to out-of-sample behavior by constructing an RKHS ambiguity set centered at the empirical \gls{cme}. The result is a distributionally robust estimator with an adjustable robustness radius. The details are omitted here for brevity, but the interested reader is referred to~\citet{kuhn2025distributionally,li2022optimal}.


\subsection{Data-Driven Spectral Barriers}\label{sec:ddbarriers}
Based on the data-driven estimator in \eqref{eq:empiricalEstimate}, the problem of computing \glspl{cbc} based on data can be formulated as a nonconvex semi-infinite program, which for general classes of systems and barriers is extremely difficult to solve.
...
Partitioning the space of spatial frequencies $\omega\in\R^n$ into a set of discrete frequency bands (see Figure~\ref{fig:spectral_barrier}) yields a spectral abstraction of the associated RKHS, i.e., characterizing learnable functions $B$ in the form of Fourier series.
...
\end{align*}
Notably, $\B$ admits the linear form $B(x)=b\T\phi_M(x)$, with a truncated Fourier feature map $\phi_M\colon\X\rightarrow\R^{2M+1}$, parametrized by learnable spectral amplitudes
\begin{figure}
    \centering
...
\begin{align*}
    b\!=\!\begin{bmatrix}
              \frac{\alpha_0}{\sigma_f^2w_0^2}\! & \frac{\alpha_1}{2\sigma_f^2w_1^2}\!\! & \frac{\beta_1}{2\sigma_f^2w_1^2}\!\! & \ldots\!\! & \frac{\alpha_{M}}{2\sigma_f^2w_{M}^2}\!\! & \frac{\beta_{M}}{2\sigma_f^2w_{M}^2}
...
The \gls{cme}-based estimator \eqref{eq:empiricalEstimate} is approximated in the same finite basis, with an approximation error decreasing exponentially with $M$~\cite{rahimi2007random}.
This simplifies the barrier problem to a semi-infinite linear program.
...
As a result, the semi-infinite problem is relaxed to a finitely-constrained \gls{lp}, provided in full in {\color{red}the appendix}.
Recall that given an appropriate robustness radius, the barriers produced by \lucid based on data $\data_N$ are statistically correct with respect to the unknown true system $\S$.
...
In its current form, \lucid implements all functionality needed to certify safety of closed-loop system with a given control policy $\pi$.
Future releases will expand this core functionality to synthesize controllers that render the system safe as well.
Due to its modular architecture, \lucid can be easily extended in multiple directions, as outlined in Section~\ref{sec:future_extensions}.
...
The samples can be specified directly in the configuration file:
...
\end{lstlisting}
Alternatively, the samples can either be generated programmatically in the user-provided Python configuration script, or be stored in a separate file.
\pylucid natively supports common formats such as \texttt{.csv}, \texttt{.mat}, \texttt{.npy}, and \texttt{.npz}.
...
However, they still depend on a number of \hp, such as the kernel bandwidth $\sigma_f$, the lengthscale $\sigma_l$, and the regularization constant $\lambda$.
...
\begin{itemize}
    \item \texttt{MedianHeuristicTuner} uses the \emph{median heuristic} \cite{garreau2018largesampleanalysismedian} to produce rule-of-thumb-type estimates for the hyperparameters $\sigma_f$ and $\sigma_l$ of, e.g., the Gaussian kernel \eqref{eq:gaussian_kernel}, via closed-form expressions.
          This can offer a cheap starting point for successive fine-tuning.
    \item \texttt{LbfgsTuner} finds the \hp that maximize the \emph{log marginal likelihood}~\cite{rasmussen2006gaussian}, defined as
          ...
          The \texttt{LbfgsTuner} uses the L-BFGS or L-BFGS-B quasi-Newton optimization algorithms~\cite{book:nonlinear-programming},
          implemented in the \texttt{LBFGS++} library\footnote{https://github.com/yixuan/LBFGSpp}.
    \item \texttt{GridSearchTuner} implements the grid search method, exploring the space of possible \hp values to find the one that maximizes the \estimator's $R^2$ score, defined as $R^2 = 1 - (\sum_{i=1}^M (y_i - \hat{y}_i)^2/\sum_{i=1}^M (y_i - \bar{y})^2)$.
\end{itemize}
...
def scenario_config(c: Configuration):
    ...
    return c
\end{lstlisting}
For the running example, we set the hyperparameters explicitly:
\begin{lstlisting}[language=yaml,backgroundcolor=\color{ipython_bg}]
sigma_f: 15
sigma_l: [1.76]
lambda: 1.0e-5
...
We exploit the truncated spectral kernel expansion in \eqref{eq:sqexp_kernel_fourier} by building an explicit approximated feature map, composed of a linear sum of trigonometric functions with increasing frequencies (see Figure~\ref{fig:spectral_barrier}).
...
In our example, we truncate the Fourier expansion to $5$ frequency bands, including the constant term, and use $64$ times the Nyquist frequency of lattice points to build the constraints of the optimization problem, to avoid it being too conservative.\OS{Specifying it as a multiple of the Nyquist frequency will inevitably raise questions. Just say we generate $X$ lattice points? "lattice density"}
\begin{lstlisting}[language=yaml,backgroundcolor=\color{ipython_bg}]
feature_map: 
...
Generating the \gls{lp}, specifically choosing an appropriate lattice density, involves trading off efficiency versus conservativeness (see Section~\ref{sec:ddbarriers}), with any sampling density beyond the Nyquist frequency yielding a valid relaxation.
...
For the running example, we obtain the barrier certificate shown in Figure~\ref{fig:example} for a control horizon of $T = 15$ certifying safety of the system in \eqref{eq:linear-system} with a probability of at least $93.73\%$ ($\eta = 0.013, \gamma=1.0, c = 0.003$). %\|B\| = 1.72

\begin{figure}
    \centering
    \resizebox{\columnwidth}{!}{\import{figures}{example.pgf}}
    \caption{Barrier certificate for the linear system example.
        The state space $\X$ is plotted in the X axis, while the value of the barrier $\B$ is plotted in the Y axis.
        The green line represents the barrier value at time $t$ while the purple one represents the barrier value at time $t+1$.
...
Note that \dreal has to be installed separately with \lstinline|pip install dreal| and may not be available on all systems.
...
verify: true
\end{lstlisting}
Doing so confirms that the $\B$ is indeed a valid \gls{cbc} for the system.
...
Running \lstinline|pylucid-gui| will open a browser tab containing the interface shown in Figure~\ref{fig:gui}, while a local server will listen for requests coming from the \gls{gui}, computing and returning the results.
\todo{Improve picture of the GUI}
\begin{figure}
    \centering
    \includegraphics[trim={0 12.4cm 15cm 0},clip,width=.7\linewidth]{figures/frontend.png}
    \caption{\lucid's \gls{gui} can be accessed via any web browser. 
    The left half of the screen allows the user to provide the transition samples and the safety specification to verify.
    The results will be shown on the right half of the screen.
    }
    \label{fig:gui}
\end{figure}
...
To ascertain the performance of \lucid, we conducted a series of experiments, run on a Windows 10 machine with an AMD Ryzen 9 5950X 16-Core Processor @ 3.40 GHz, and 64 GB of RAM.
All runs had the random seed set to $42$ to ensure reproducibility.

\subsection{Barrier 2}

The \barrII experiment is inspired by the one from \cite{abate2021fossil}, which we extend by adding stochastic noise $w_t\sim\mathcal{N}(\cdotx\vert 0,0.01I_2)$ to arrive at the two-dimensional nonlinear stochastic dynamics
...
\EC{we can remove all the explicit set intevals}
The results are shown in Figure~\ref{fig:CSBarr2}.
The kernel \hp have been selected through offline tuning to be $\sigma_f=X$ and $\sigma_l=[X, X]$.
Using a spectral basis of $M=X^3-1$ frequencies and a lattice density of $\hat{N} = X^2$, 
we were able to generate a \gls{cbc} with $\eta \approx X.X$, $\gamma \approx X.X$, and $c \approx X.X$ in $X$ minutes and $X$ seconds, achieving a safety probability of $P_{safe}(\S^\pi) \approx XX.X\%$.
% Knowing the system dynamics, we were also able to formally validate the barrier.

\begin{figure}[ht]
    \resizebox{\columnwidth}{!}{\import{figures}{barrier3.pgf}}%
    \caption{\gls{cbc} synthesis for the \barrIII benchmark.
...
The \barrIII experiment is inspired by the one from \cite{abate2021fossil}, which we extend by adding stochastic noise $w_t\sim\mathcal{N}(\cdotx\vert 0,0.01I_2)$ to arrive at the two-dimensional nonlinear stochastic dynamics
\begin{equation*}
    \begin{bmatrix}
        {x}_{1, t+1} \\
        {x}_{2, t+1}
...
\end{align*}
we want to ensure that the system, starting in $\X_0$, does not enter the unsafe regions $\X_U$ within $T=10$ time steps.
\EC{we can remove all the explicit set intevals}
This example is particularly challenging due to the closeness between the initial set and the unsafe set.
Hence, we need a rather high number of frequencies to achieve the required expressivity in the barrier.
The results are shown in Figure~\ref{fig:CSBarr3}.
The kernel \hp have been selected through offline tuning to be $\sigma_f=X$ and $\sigma_l=[X, X]$.
Using a spectral basis of $M=X^3-1$ frequencies and a lattice density of $\hat{N} = X^2$, 
we were able to generate a \gls{cbc} with $\eta \approx X.X$, $\gamma \approx X.X$, and $c \approx X.X$ in $X$ minutes and $X$ seconds, achieving a safety probability of $P_{safe}(\S^\pi) \approx XX.X\%$.
% Knowing the system dynamics, we were also able to formally validate the barrier.
...
We consider a scenario where an autonomous vehicle (AV) controlled by a \gls{nn} is overtaking another vehicle, as indicated in Figure~\ref{fig:safety_setup}. 
...
The goal is to certify that the controlled system remains safe by computing the probability that the system initialized in $[{x}_0,{y}_0,\phi_0]\T\in \X_0$ does not enter the unsafe region $\X_u$ within $T=50$ time steps.

The \gls{lp} is generated based on a lattice of $\hat{N}=X^3$ equidistant points and a spectral basis of $M=4^3-1$ frequencies.
The kernel \hp are selected to be $\sigma_f=X$ and $\sigma_l=[X, X, X]$.
We obtained a barrier which certifies safety with a probability of at least $XX\%$ with $c=0$, $\gamma=X.X$ and $\eta=X.X$.
Note that this is only certifying safety w.r.t. the empirical distribution, i.e., the \gls{cme} constructed from the observed data.
...
\begin{table}[tb]
    ...
    \begin{tabular}{ccccc}
        \toprule
        \textbf{Benchmark} & \textbf{Dim} & \textbf{Lattice} & \textbf{Runtime} & \textbf{Safety Prob.} \\
                           &              & \textbf{Density} & [mm:ss]          & [\%]                  \\ % Units
        \midrule
        Linear             & 1            & $?$              & 00:02            & 95.15                 \\
        Barr3              & 2            & $30^2$           & XX:XX            & XX                    \\
                           &              & $40^2$           & XX:XX            & XX                    \\
                           &              & $?^2$            & XX:XX            & XX                    \\
        Overtaking         & 3            & $?^3$            & XX:XX            & XX                    \\
                           & 3            & $?^3$            & XX:XX            & XX                    \\
                           & 3            & $?^3$            & XX:XX            & XX                    \\
        name2              & 7            &                  & XX:XX            & XX                    \\
        \bottomrule
...
\OS{As the GaussianKernel and KernelRidgeRegressor are the only options we currently provide, is specifying them explicitly necessary? Maybe we could also add a sentence about the fact that they are currently the only option, but that in the future we plan to extend this to to allow to include prior knowledge about the system dynamics (in the spirit of physics-informed approaches).}
