...
\paragraph{Dataset}
Due to their complexity and opacity, such systems must often be treated as black boxes in their entirety, assuming only access to a finite amount, $N$, of system observations of the form
\begin{equation}
...
A high-level visualization of \lucid's architecture is illustrated in Figure~\ref{fig:architecture}, while a more technical description can be found in the online documentation at \url{https://lucidtoolsource.gitlab.io/lucid/}.
...
Hence, for rest of the paper, we will focus on \pylucid when describing the user interfaces.
...
To define the configuration, we suggest using a \yaml or equivalent \json file.
...
If more flexibility is needed, a Python configuration script can be specified instead,
...
Regardless of their format, configuration files will then be loaded with the command \lstinline|pylucid <config file>| to start the tool's verification pipeline.
...
To make the explanation more intuitive, we will use the example system characterized by
\begin{equation}
    \label{eq:linear-system}
    \LS\colon\begin{bmatrix}
        {x}_{1, t+1}
    \end{bmatrix}
    = \begin{bmatrix}
        0.5
...
While going over each component, we will show how they can be configured within a \yaml configuration file to capture the system~\eqref{eq:linear-system} and its safety specification.
...
The easiest way to provide the samples is to add the to the configuration file
...
\end{lstlisting}
\end{comment}
\EC{Explaining in details how the configuration file works would take quite a bit of space.}
% \todo{HDF5?}
...
\lucid understands \texttt{RectSet}s, \texttt{SphereSet}s, and \texttt{Multiset}s, i.e., collections of sets.
...
\begin{itemize}
    \item \texttt{MedianHeuristicTuner}: it uses the \emph{median heuristic} \cite{garreau2018largesampleanalysismedian}, to produce initial estimates for the hyperparameters $\sigma_f$ and $\sigma_l$.
          Can be a cheap starting point when working with a Gaussian kernel as in~\eqref{eq:gaussian_kernel}.
    \item \texttt{LBFGSTuner}: borrowing the technique from Gaussian processes, we optimize the \hp to maximize the \emph{log marginal likelihood}, defined as
          \begin{equation}
              \begin{split}
                   & \log p(X^+_N | X_N,\sigma_l) =  - \frac{1}{2}X^+_N{}\T (K_{X}^N + N\lambda I_N)^{-1} X^+_N \\
                   & \hspace{40pt} -\frac{1}{2}\log |K_{X}^N + N\lambda I_N| - \frac{N}{2}\log(2\pi),
              \end{split}
          \end{equation}
          where $\sigma_l$ is the hyperparameter being optimized.
    ...
    \item \texttt{GridSearchTuner}: implements the grid search method, exploring the space of possible \hp values to find the one that yields the best \estimator $R^2$ score defined as $R^2 = 1 - \frac{\sum_{i=1}^n (y_i - \hat{y}_i)^2}{\sum_{i=1}^n (y_i - \bar{y})^2}$.
\end{itemize}
Tuners open a wide range of possibilities to the user.
We recommend using them within a Python configuration script to automate the tuning process before finalizing the \estimator's \hp values.
...
There are currently three different ways the probability distribution can be divided in when associated to the frequency bands as shown in Figure~\ref{fig:spectral_measure_abstraction}:
\begin{itemize}
    \item \texttt{LinearTruncatedFourierFeatureMap}: the frequency bands are equally spaced, capturing the interval $[-3\sigma_l,3\sigma_l]$.
    \item \texttt{LogTruncatedFourierFeatureMap}: the frequency bands are spaced logarithmically, capturing the interval $[-3\sigma_l,3\sigma_l]$.
    \item \texttt{ConstantTruncatedFourierFeatureMap}: the frequency bands have a fixed size.
\end{itemize}
Different feature map implementations may yield different results.
In our example, we truncate the Fourier expansion to $5$ terms, including the constant term, and use $64$ times the Nyquist frequency of lattice points to build the constraints of the optimization problem, to avoid it being too conservative.
\begin{lstlisting}[language=yaml]
feature_map: |
...
The finite-constraint relaxation of the SIP \eqref{eq:semiinf-prog} is what \lucid uses to synthesize the \glspl{cbc}.
...
\lucid can be configured to work with any of the following linear optimizers: \gurobi, \alglib or \highs as shown below.
\begin{lstlisting}[language=yaml]
...
The solution was verified with \dreal, ensuring its formal correctness.
The plot is shown in Figure~\ref{fig:example}.
...
if the system dynamics are known, \lucid provides the option to formally verify its correctness using the \dreal SMT solver~\cite{dreal}.
Note that \dreal has to be installed separately with \lstinline|pip install dreal| and may not be available on all systems.

Since we know the expected behavior of the example system $\LS$ from~\eqref{eq:linear-system}, we can run the formal validation by adding the following two lines to the configuration
\begin{lstlisting}[language=yaml]
