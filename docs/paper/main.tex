...
The empirical estimator in \eqref{eq:empiricalEstimate} converges in expectation to the true CME for $N\rightarrow\infty$~\cite{park2020measuretheoretic}. 
...
The CME-based estimator \eqref{eq:empiricalEstimate} is approximated in the same finite basis, with an approximation error decreasing exponentially with $M$~\cite{rahimi2007random}.
...
As a result, the semi-infinite problem is relaxed to a finitely-constrained linear program (LP), provided in full in {\color{red}the appendix}.
...
The core of \lucid is its \estimator, namely the \texttt{KernelRidgeRegressor}, which uses the \texttt{GaussianKernel} to learn the underlying system dynamics by estimating the CME from the samples.
Its predictions of the expected next state $x_+$ are used
% of all the lattice points we need from $\X$, necessary 
to determine the constraints for the \gls{cbc} LP.
...
Based on the components discussed above, \lucid generates and solves a finite-constraint LP as outlined in Section~\ref{sec:ddbarriers}. 
...
Generating the LP, specifically choosing an appropriate lattice density, involves trading off efficiency versus conservativeness (see Section~\ref{sec:ddbarriers}), with any sampling density beyond the Nyquist frequency yielding a valid relaxation.
...
For solving the LP, \lucid can interface with different linear optimizers: \gurobi, \alglib or \highs. Here, we use \gurobi:
\begin{lstlisting}[language=yaml,backgroundcolor=\color{ipython_bg}]
...
For the running example, we obtain the barrier certificate shown in Figure~\ref{fig:example} for a control horizon of $T = 15$ certifying safety of the system in \eqref{eq:linear-system} with a probability of at least $95.15\%$ ($\eta = 0.257, \gamma=?, c = 0.031$). %\|B\| = 1.72
...
\begin{figure*}
    ...
    \caption{The GUI of \lucid with the main functions highlighted. \EC{Being originally full page, shrinking it to half a column makes the immage illegible. Should I zoom on some parts?}}
    \label{fig:gui}
\end{figure*}
