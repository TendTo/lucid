...
\end{lstlisting}
If the desired platform's wheel is not available, \pylucid can be installed from source by cloning the repository and running
\begin{lstlisting}[language=bash,numbers=none,xleftmargin=0em]
...
\begin{figure}
    ...
    \includegraphics[trim={0 12.4cm 15cm 0},clip,width=.7\linewidth]{figures/frontend.png}
    \caption{\lucid's \gls{gui} can be accessed via any web browser. 
    The right half of the screen shows the results produced by \lucid, including a plot of the computed barrier function, 
    the numerical safety guarantee and , and detailed logs of the computations.
    }
    \label{fig:gui-right}
\end{figure}
